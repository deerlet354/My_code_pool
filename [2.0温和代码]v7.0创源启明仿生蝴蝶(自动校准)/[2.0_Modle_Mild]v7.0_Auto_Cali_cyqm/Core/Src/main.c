...
uint8_t M_pos[4]={NONE, NONE, NONE, NONE};
uint8_t E_pos[4]={NONE, NONE, NONE, NONE};

// 左右电机的基准点（中位角度//调小翅膀向上）
int16_t motor_L_midpoint   = 2000;
int16_t motor_R_midpoint   = 2000;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	Get_cali_data();			//从flash读取校准数据
	ELRS_Init();			//接收机初始化
	Chassis_PID_Init(); 	//电机PID初始化
	StarAndGetResult();
	last_ad[0] = AD_Value[0];
	last_ad[2] = AD_Value[2];
  /* USER CODE END 2 */
  ...
  while (1)
  {
	...
	now = HAL_GetTick();
	a = now;
	 if(elrs_data.Switch_L == 0&&elrs_data.Switch_R == 0)//校准完成后两边开关关闭重启
     {
		if(state == Cali_ok)
    ...
    else if (elrs_data.Switch_L == 1&&elrs_data.Switch_R == 0)
    {

        if (elrs_data.Mode_L == 2)
        {
			motor_1_pid.Kp = 30;
			motor_3_pid.Kp = 30;
			const int16_t motor_L_ready		=	motor_L_midpoint	+	elrs_data.midpoint*2	+	elrs_data.midpoint_1*2;
			const int16_t motor_R_ready		=	motor_R_midpoint	+	elrs_data.midpoint*2	+	elrs_data.midpoint_1*2;

            // 幅值（保持与原设计一致：Pitch ± Yaw，整型）
            const int16_t ampR = (int16_t)(800 + elrs_data.Yaw*3);			
            const int16_t ampL = (int16_t)(800 - elrs_data.Yaw*3);
			thr = 12;
			// 一个周期 30 步（15 去 + 15 回）
...
//			Motor_PID_Control();
	...
	else if(elrs_data.Switch_R == 1&&elrs_data.Switch_L == 0)//进入校准模式，需要把电机臂放平
	{
		...
		if(state == Default)
		{
			for(uint8_t i = 0; i < 4; i++)
			{
				E_pos[i] = NONE;
				M_pos[i] = NONE;
				Wings_Data.Wings_motor[i].encoder_dir = 0;
				Wings_Data.Wings_motor[i].motor_dir = 0;
				Wings_Data.Wings_motor[i].MOTOR_MIDPOINT = 0;
			}
		}
		
		int16_t ad_diff[4];
		static int16_t Relative_Angle[6];
		ad_diff[0] = (int16_t)AD_Value[0] - last_ad[0];
		ad_diff[2] = (int16_t)AD_Value[2] - last_ad[2];
		Relative_Angle[0] += Encoder_unwrap(ad_diff[0]);
		Relative_Angle[2] += Encoder_unwrap(ad_diff[2]);
		
		if(elrs_data.Mode_R == 2)//编码器校准，先将左翅立起来，后将右翅立起来
		{
			state = Encoder_cali;
			motor_disable();
			if(!(Wings_Data.Wings_motor[0].encoder_dir && Wings_Data.Wings_motor[2].encoder_dir))
			{
				...
				switch(num)
				{
					case 0:
					{
						for(uint8_t i=0;i<4;i++)
						{
							...
							if(abs(Relative_Angle[i])>=500)
							{
								E_pos[0] = i;
								Wings_Data.Wings_motor[0].encoder_dir = Get_encoder_dir(Relative_Angle[i]);
								Relative_Angle[i] = 0;
								num++;
							}
						}
					...
					case 1:
					{
						for(uint8_t i=0;i<4;i++)
						{
							...
							{
								E_pos[2] = i;
								Wings_Data.Wings_motor[2].encoder_dir = Get_encoder_dir(Relative_Angle[i]);
								Relative_Angle[i] = 0;
								num++;
							}
						}
...
//					}
			...
			Relative_Angle[E_pos[0]] = 0;
			Relative_Angle[E_pos[2]] = 0;

		}
		else if(elrs_data.Mode_R == 1)//把两边翅膀打平
		{
			if(!(Wings_Data.Wings_motor[0].MOTOR_MIDPOINT&&Wings_Data.Wings_motor[2].MOTOR_MIDPOINT))
			{
				Wings_Data.Wings_motor[0].MOTOR_MIDPOINT = Get_correct_ad(Wings_Data.Wings_motor[0].encoder_dir, AD_Value[E_pos[0]]);
				Wings_Data.Wings_motor[2].MOTOR_MIDPOINT = Get_correct_ad(Wings_Data.Wings_motor[2].encoder_dir, AD_Value[E_pos[2]]);
			}
			
			if(Wings_Data.Wings_motor[0].Corrective_Angle>=1500&&Wings_Data.Wings_motor[0].Corrective_Angle<=2500
				&&Wings_Data.Wings_motor[2].Corrective_Angle>=1500&&Wings_Data.Wings_motor[2].Corrective_Angle<=2500)
			{
				state = Motor_cali;
			}
		}
		else if(elrs_data.Mode_R == 0)//电机校准，两边电机停止运动后校准完成
		{
			if(state == Motor_cali)
			{
				static uint8_t num = 0;
				switch(num)
				{
					case 0:
					{
						...
						for(uint8_t i=0;i<4;i++)
							if(Compare(Wings_Data.Wings_motor[i].Corrective_Angle, 2700, 1300))
							{
								M_pos[0] = i;
								Wings_Data.Wings_motor[i].motor_dir = Compare(Wings_Data.Wings_motor[i].Corrective_Angle, 2700, 1300);
								TIM2->CCR1=1000;TIM2->CCR2=1000;
								num++;
							}
						break;
					}
...
//					}
				...
				if(Wings_Data.Wings_motor[0].motor_dir && Wings_Data.Wings_motor[2].motor_dir)
					state = Flash_write;
			}

			if(state == Flash_write)//写入flash
			{
				Save_cali_data();
				state = Cali_ok;
			}
