...
void ELRS_UARTE_RxCallback(uint16_t Size)
{
    ...
    if (elrs_data_temp[date_i+0] == CRSF_ADDRESS_FLIGHT_CONTROLLER)
    {
        ...
        if (elrs_data_temp[date_i+2] == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) // 数据帧类型为RC通道数据
        {
            ...
            elrs_data.Switch_L 	= ( elrs_data.channels[4] < 1050 ) ? 0 :( elrs_data.channels[4] > 1700	 ) ? 1 : 0;
			elrs_data.Mode_L 		= (elrs_data.channels[5] > 1700) ? 2 :((elrs_data.channels[5] > 900 && elrs_data.channels[5] < 1100) ? 1 : 0);
			elrs_data.Switch_R = ( elrs_data.channels[7] < 1050 ) ? 0 :( elrs_data.channels[7] > 1700 ) ? 1 : 0;
			elrs_data.Mode_R = (elrs_data.channels[6] > 1700) ? 2 : ((elrs_data.channels[6] > 900 && elrs_data.channels[6] < 1100) ? 1 : 0);
		}
