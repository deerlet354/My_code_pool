...
{
//	
	Wings_Data.Wings_motor[0].Target_Speed=motor_pwm[0]=PID_calc(&motor_1_pid, Wings_Data.Wings_motor[0].Corrective_Angle , Wings_Data.Wings_motor[0].Target_Angle) * Wings_Data.Wings_motor[0].motor_dir;
	Wings_Data.Wings_motor[2].Target_Speed=motor_pwm[2]=PID_calc(&motor_3_pid, Wings_Data.Wings_motor[2].Corrective_Angle , Wings_Data.Wings_motor[2].Target_Angle) * Wings_Data.Wings_motor[2].motor_dir;
	
	Set_Pwm(motor_pwm[M_pos[0]], motor_pwm[M_pos[1]], motor_pwm[M_pos[2]], motor_pwm[M_pos[3]]);
}
...
int16_t Encoder_unwrap(int16_t value)
{
	if(value >= 3072)
		...
		return value + 4095;
	else if(abs(value)>100&&abs(value)<3072)
		return 0;
	else
		return value;
