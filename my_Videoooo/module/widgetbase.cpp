...
VWidgetBase::VWidgetBase(QWidget *parent) : QWidget(parent)
{
    FUNCTION_ENTER;
    m_titleBar = NULL;
    InitBase();
...
VWidgetBase::~VWidgetBase()
{
    FUNCTION_ENTER;
    FUNCTION_EXIT;
}

void VWidgetBase::setTitleBar(QWidget *titleBar)
{
    FUNCTION_ENTER;
    m_titleBar = titleBar;
...
bool VWidgetBase::eventFilter(QObject *watched, QEvent *event)
{
...
bool VWidgetBase::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
    //FUNCTION_ENTER;
    bool bRet = false;
...
void VWidgetBase::InitBase()
{
    ...
    //SetWindowDisplayAffinity(hwnd, true);
#endif
    FUNCTION_EXIT;
}

bool VWidgetBase::winMouseEvent(MSG *msg, long *result)
{
    // 获取鼠标坐标
...
void VWidgetBase::thisEvent(QObject *watched, QEvent *event)
{
    if(watched != this)
    {
        return;
    }
    //qDebug() <<"env type " << event->type();
